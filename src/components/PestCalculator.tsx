...
export function PestCalculator({ onOrderRecommended }: PestCalculatorProps) {
  ...
  const [pestType, setPestType] = useState<'Bedbugs' | 'Cockroaches' | 'Both'>('Bedbugs');
  ...
  const estimatedCostNgn = (recommended500ml * 3000) + (recommended250ml * 2000);
  ...
  return (
    <div className="relative glass-card rounded-3xl p-6 md:p-8 border border-slate-200 shadow-2xl overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        ...
        <div className="lg:col-span-7 space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {(['Bedbugs', 'Cockroaches', 'Both'] as const).map((p) => (
                <button
                  key={p}
                  type="button"
        ...
        <div className="lg:col-span-5 glass-card p-6 rounded-2xl border border-red-500/40 bg-gradient-to-b from-red-50 to-white">
          ...
          <button
            onClick={() => onOrderRecommended(500, recommended500ml + recommended250ml)}
            className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-red-600 to-amber-500 text-white font-bold text-xs uppercase tracking-wider shadow-lg shadow-red-600/40 hover:brightness-110 active:scale-98 transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
            <ShoppingBag className="w-4 h-4" />
            <span>Order Estimated Package</span>
          </button>
        </div>
      </div>
    </div>
